...
def check_intersecting(cubes, ranges_to_check, status_to_check):
    ...
    for cube_ranges in cubes:
        is_x_intersecting = check_intersect(cube_ranges[0], cube_ranges[1], ranges_to_check[0],
                                            ranges_to_check[1]) or check_inside(cube_ranges[0], cube_ranges[1],
                                                                                ranges_to_check[0],
                                                                                ranges_to_check[1]) or check_inside(
            ranges_to_check[0], ranges_to_check[1], cube_ranges[0], cube_ranges[1])
        is_y_intersecting = check_intersect(cube_ranges[2], cube_ranges[3], ranges_to_check[2],
                                            ranges_to_check[3]) or check_inside(cube_ranges[2], cube_ranges[3],
                                                                                ranges_to_check[2],
                                                                                ranges_to_check[3]) or check_inside(
            ranges_to_check[2], ranges_to_check[3], cube_ranges[2], cube_ranges[3])
        is_z_intersecting = check_intersect(cube_ranges[4], cube_ranges[5], ranges_to_check[4],
                                            ranges_to_check[5]) or check_inside(cube_ranges[4], cube_ranges[5],
                                                                                ranges_to_check[4],
                                                                                ranges_to_check[5]) or check_inside(
            ranges_to_check[4], ranges_to_check[5], cube_ranges[4], cube_ranges[5])

        if is_x_intersecting and is_y_intersecting and is_z_intersecting:
            split_x_ranges, split_y_ranges, split_z_ranges, ranges = get_ranges_from_intersecting_cubes(cube_ranges,
                                                                                                        ranges_to_check)
...
if __name__ == '__main__':
    ...
    while len(list(cubes_to_check.keys())) > 0:
        volume = sum(
            [len(range(cube[0], cube[1] + 1)) + len(range(cube[2], cube[3] + 1)) + len(range(cube[4], cube[5] + 1)) for
             cube in cubes])
